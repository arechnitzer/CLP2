...
which is in Example~\eref{CLP101}{eg:SRpTest} in the CLP-2 text, the series
$\displaystyle \sum_{n=5}^\infty \frac{1}{n^p}$ converges for all $p>1$. So we would
expect the given series to converge absolutely.

Now, to confirm that our intuition is correct, we'll first try the limit comparison theorem, which is Theorem~\eref{CLP101}{thm:SRlimitComparison} 
...
Theorem~\eref{CLP101}{thm:SRcomparisonTest} in the CLP-2 text, if
