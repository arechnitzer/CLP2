...
\item The first sample, as given in the question statement, is taken at $x=a$. The second sample, then, is at $x=a+\frac{b-a}{n}$, the third is at $x=a+2\frac{b-a}{n}$, and the fourth is at $a+3\frac{b-a}{n}$.
...
\begin{Mquestion}
The carbon dioxide concentration in the air at a particular location at time $t$, during the course of one year, is approximated by \[C(t) = 400+50\cos\left(\frac{t}{12}\pi\right)+200\cos\left(\frac{t}{4380}\pi\right)\] parts per million, where  $t$ is measured in hours from the beginning of the year. 
The middle term, $50\cos\left(\frac{t}{12}\pi\right)$, has a period of $24$ hours, i.e. one day. It gives variations
over the course of a day.
The final term, $200\cos\left(\frac{t}{4380}\pi\right)$, has a period of $8760$ hours, i.e. one year. It gives 
seasonal variations over the course of a year.
\begin{enumerate}[(a)]
\item  What is the average carbon dioxide concentration for that location for that year?
\item What is the average over the first day? 
\item At noon of every day, i.e. when $t=12+24n$, where $n$ is any whole number between 0 and 364,
the daily variation term takes the same value:
\[ 
50\cos\left(\frac{(12+24n)}{12}\pi\right) = 50\cos\left(\pi+2\pi n\right) = 50\cos\pi=-50
\]
Suppose we construct another approximation for the concentration of carbon dioxide in the atmosphere by replacing the 
middle, daily variation, term with its noon value, $-50$:
\[N(t) = 350 +200\cos\left(\frac{t}{4380}\pi\right)\quad\text{ ppm}\]
What is the relative error in the yearly average concentration of carbon dioxide introduced by using $N(t)$, instead of $C(t)$?
\end{enumerate}
...
\noindent (a) Using the definition of an average, the average concentration over one year ($t=0$ to $8760$) is:
\begin{align*}
...
\intertext{Since $\frac{8760}{12} = 730$, which is even, $\sin\left(\frac{8760}{12}\pi\right)=\sin(730\pi)=0$. Also, $\sin\left(\frac{8760}{4380}\pi\right)=\sin(2\pi)=0$. }
...
That is: replacing $C(t)$ with $N(t)$ leads to an error of 12.5\% in the yearly average concentration of carbon dioxide.
\end{solution}
...
\end{tikzpicture}
\end{center}
As in Example~\eref{CLP101}{eg:peakvsrms} of the CLP-2 text, the root mean square of the function $f(x)$ on $[a,b]$ is $\displaystyle\sqrt{\frac{1}{b-a}\int_a^b f^2(x)\,\dee{x}}$.
\end{question}
\begin{hint}
