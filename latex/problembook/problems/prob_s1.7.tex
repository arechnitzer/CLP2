...
&\Leftrightarrow& \int\diff{}{x}\{f(g(x))\}\dee{x}
                          &=\int f'(g(x))g'(x)\dee{x}\\
...
&\Leftrightarrow&\int\diff{}{x}\{f(x)g(x)\}\dee{x}
              &=\int f'(x)g(x)+f(x)g'(x)\dee{x}\\
...
\end{alignat*}
In the last line, the ``$+C$'' has been absorbed into the indefinite
...
\end{hint}
\begin{answer}
$\displaystyle\int \frac{f'(x)}{g(x)}~\dee{x}= \dfrac{f(x)}{g(x)} +\displaystyle\int\frac{f(x)g'(x)}{g^2(x)}\dee{x}$
\end{answer}
\begin{solution}
We'll use the same ideas that led to the method of integration by parts. (You can review this in your text, or see the solution to Question~\ref{1.7_difftoint} in this section.)
According to the quotient rule,
\begin{align*}
\diff{}{x}\left\{\dfrac{f(x)}{g(x)}\right\} &= \frac{g(x)f'(x)-f(x)g'(x)}{g^2(x)}.
...
This is exactly the integration by parts formula for the functions $u=1/g$ 
and $v=f$. 
\end{solution}
...
Note: compare this to Example~\eref{CLP101}{eg:PRTSlnx} of the CLP-2 text, where we chose $u=f(x)$, $\dee{v}=1\dee{x}$.
\end{question}
\begin{hint}
What integral do you have to evaluate, after you plug in your choices to the integration by parts formula?
\end{hint}
\begin{answer}
Suppose we choose $\dee{v} = f(x)\dee{x}$, $u=1$. Then $v = \displaystyle\int f(x)\dee{x}$, and $\dee{u}=0\dee{x}$. So, our integral becomes:
\begin{align*}
\int \underbrace{(1)}_{u}\underbrace{f(x)\dee{x}}_{\dee{v}}&=
\underbrace{(1)}_{u}\underbrace{\int f(x)\dee{x}}_{v} - \int\underbrace{ \left(\int f(x)\dee{x}\right)}_{v}\underbrace{0\dee{x}}_{\dee{u}} \\
&=\int f(x)\dee{x}
\end{align*}
So, in this case, using integration by parts has done absolutely nothing! 
\end{answer}
\begin{solution}
Suppose we choose $\dee{v} = f(x)\dee{x}$, $u=1$. Then $v = \displaystyle\int f(x)\dee{x}$, and $\dee{u}=0\dee{x}$. So, our integral becomes:
\begin{align*}
...
&=\int f(x)\dee{x}
\end{align*}
So, in this case, using integration by parts has done absolutely nothing! It has not eased your pain.

We note here that in certain cases, such as $\int\log x~ \dee{x}$ (Example~\eref{CLP101}{eg:PRTSlnx}  in the CLP-2 text), it is useful to choose $\dee{v}=1\dee{x}$. This is similar to, but crucially different from, the do-nothing method in this problem.
\end{solution}
...
Again, we remember that logarithm has some antiderivative we found in Example~\eref{CLP101}{eg:PRTSlnx} of the CLP-2 text, but it was something complicated. Luckily, we don't need to bother with it: when we multiply the red equations in Option 1, we get a perfectly workable integral.
...
\end{question}
\begin{hint}
This can be solved using the same ideas as Example~\eref{CLP101}{eg:PRTSlnx}  in the CLP-2 text.
\end{hint}
\begin{answer}
...
\begin{description}
\item[Solution 1:]  Following Example~\eref{CLP101}{eg:PRTSlnx} in the CLP-2 text,
...
You can use a similar trick as the book did when antidifferentiating other inverse trigonometric functions in Example~\eref{CLP101}{eg:invtan} of the CLP-2 text.
\end{hint}
...
\begin{solution}
In Example~\eref{CLP101}{eg:invtan} of the CLP-2 text, we saw that integration by parts was useful when the integrand has a derivative that works nicely when multiplied by $x$. We use the same idea here.
Let $u = \arccos y$ and $\dee{v} = \dee{y}$,
so that $v=y$ and $\dee{u} = -\frac{\dee{y}}{\sqrt{1-y^2}}$.
\begin{align*}
\int\arccos  y\,\dee{y}
...
so that $v=2y^2$ and $\dee{u}= \frac{2\, \dee{y}}{1+(2y)^2}$:
\begin{align*}
...
\end{answer}
\begin{solution}
We've got an integrand that consists of two functions multiplied together, and no obvious substitution. So, we think about integration by parts. Let's consider our options. Note in Example~\eref{CLP101}{eg:invtan} of the CLP-2 text, we found that the antiderivative of arctangent is
...
\end{question}
\begin{hint}
This example is similar to  Example~\eref{CLP101}{eg:circularint} in the CLP-2 text. The functions $e^{x/2}$ and $\cos(2x)$ both do not substantially alter when we differentiate or antidifferentiate them. If we use integration by parts twice, we'll end up with an expression that includes our original integral. Then we can just solve for the original integral in the equation, without actually integrating.
\end{hint}
\begin{answer}
...
It might help to review Example~\eref{CLP101}{eg:PRTSexsinx} in the CLP-2 text.
\end{hint}
\begin{answer}
...
\item[Solution 2:] It's not clear how to antidifferentiate the integrand, but we can certainly differentiate it. So, keeping in mind the method of Example~\eref{CLP101}{eg:PRTSexsinx} in the CLP-2 text, we take $u=\sin(\log x)$ and $\dee{v}=\dee{x}$, so $\dee{u}=\frac{1}{x}\cos(\log x)\dee{x}$ and $v=x$.
\begin{align*}
...
\end{answer}
\begin{solution}
As we saw in Section~\eref{CLP101}{sec:intdef} of the CLP-2 text, there are many different ways to interpret a limit as a Riemann sum. In the absence of instructions that restrain our choices, we go with the most convenient interpretations.
