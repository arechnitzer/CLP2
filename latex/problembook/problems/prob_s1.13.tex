...
\end{align*}
Using the result of Example~\eref{CLP101}{eg:TRGINToptc} in the CLP-2 text,
or a table of integrals:
\begin{align*}
\int\dfrac{-2}{3x^2+4x+1}\dee{x}&=-2\log\left|\cot\theta-\csc\theta \right|+C
\end{align*}
...
\end{center}
\begin{align*}
-2\log\left|\cot\theta-\csc\theta \right|+C&=
-2\log\left|\dfrac{1}{\sqrt{9x^2+12x+3}}-\dfrac{3x+2}{\sqrt{9x^2+12x+3}} \right|+C\\
&=
-2\log\left|-\dfrac{3x+1}{\sqrt{9x^2+12x+3}}\right|+C
\\
&=
-\log\left|\dfrac{(3x+1)^2}{\sqrt{9x^2+12x+3}^2}\right|+C
\displaybreak[0]\\
&=
-\log\left|\dfrac{(3x+1)^2}{9x^2+12x+3}\right|+C
\displaybreak[0]\\
&=
-\log\left|\dfrac{(3x+1)^2}{3(3x+1)(x+1)}\right|+C\\
&=
-\log\left|\dfrac{(3x+1)}{3(x+1)}\right|+C
\\&=
\log\left|\dfrac{x+1}{3x+1}\right|+\log3+C
\end{align*}
Since $C$ is an arbitrary constant, we can rename $\log3+C$ to $C$ and write our final answer as 
\begin{align*}
\int\dfrac{-2}{3x^2+4x+1}\dee{x}&=~\log\left|\dfrac{x+1}{3x+1}\right|+C
\end{align*}
...
\begin{hint}
Try a substitution, as in Section~\eref{CLP101}{sec:tansec} of the CLP-2 text.%1.8.2.
\end{hint}
...
\begin{description}
\item[Solution 1:]
If we're stuck, we at least see that we have a compound function -- a function of a function. We often find it useful to substitute for the ``inside" function. So, we
...
\item[Solution 2:]
We saw, in Solution 1, that we could use a change of variables to convert the given integral
into an integral that we have already seen in Example \eref{CLP101}{eg:PRTSexsinx} of
the CLP-2 text. In fact, we can evaluate the given integral using the method of Example \eref{CLP101}{eg:PRTSexsinx}, without making a preparatory change of variables. As we did there, we will evaluate
two integrals at more or less the same time. Define
\begin{align*}
I_1=\int \sin(\log x)\dee{x}\qquad
I_2=\int \cos(\log x)\dee{x}
\end{align*}

\begin{itemize}
 \item First we apply integration by parts to $I_1$ with $u=\sin(\log x)$, $\dee{v} = \dee{x}$ so
              that $v = x$, $\dee{u}= \frac{1}{x}\cos(\log x)\dee{x}$.
\begin{align*}
I_1=\int \sin(\log x)\dee{x}
&=\int u \dee{v} = uv -\int v \dee{u} \\
&=x\sin(\log x) - \int \cos(\log x)\dee{x}
\end{align*}
We have not found $I_1$ but we have related it to $I_2$.
\begin{align*}
I_1&= x\sin(\log x) - I_2
\end{align*}
\item Now start over with $I_2$, applying integration by parts with $u=\cos(\log x)$, $\dee{v} = \dee{x}$ so
              that $v = x$, $\dee{u}= -\frac{1}{x}\sin(\log x)\dee{x}$.
\begin{align*}
I_2=\int \cos(\log x)\dee{x}
&=\int u \dee{v} = uv -\int v \dee{u} \\
...
& I_1&=-x\cos(\log x) + I_2
\end{align*}
\item So now, adding these two equations together and dividing by two gives,
recalling that every indefinite integral contains an arbitrary constant,
\begin{align*}
I_1&=\frac{x}{2}\big[\sin(\log x)- \cos(\log x)\big] -\frac{I_2}{2}+\frac{I_2}{2} \\
&=\frac{x}{2}\big[\sin(\log x)- \cos(\log x)\big] +C
\end{align*}
If you are worried about $-\frac{I_2}{2}+\frac{I_2}{2}$ being an arbitrary constant, rather than $0$,
note that the value of the arbitrary constant in the first $I_2$ need not be the same as the value 
of the arbitrary constant in the second $I_2$. 

\item
As a bonus (the question does not ask for $I_2$)
\begin{align*}
I_2 &= x\cos(\log x) + I_1 = \frac{x}{2}\big[\sin(\log x)+ \cos(\log x)\big] +C
\end{align*}
\end{itemize}
