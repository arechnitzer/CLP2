...
To determine the radius of convergence, we apply (\eref{CLP101}{eq:SRradConv}) in the CLP-2 text. To do so, we set $A_n= \frac{(-1)^n}{\sqrt{n}}$ and compute
\begin{align*}
A = \lim_{n\to\infty} \bigg| \frac{A_{n+1}}{A_n}\bigg|
...
We could determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}), from the CLP-2 text,
...
To determine the radius of convergence, we apply (\eref{CLP101}{eq:SRradConv}) in the CLP-2 text. To do so, 
...
\begin{solution}
We determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}) in the CLP-2 text. To do so, 
...
$A_n (x-c)^n$ required by (\eref{CLP101}{eq:SRradConv}). Instead, we use the alternate
...
\noindent (c)
This time we cannot determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}), from the 
CLP-2 text, because the $n^{\rm th}$ term is not in the form $A_n (x-c)^n$ required by 
(\eref{CLP101}{eq:SRradConv}). But we may still apply the ratio test with $a_n= \frac{4^n}{n}(2x+1)^{2n}$.
...
\begin{solution}
We determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}) in the CLP-2 text. 
...
\begin{solution}
We cannot determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}), from the 
CLP-2 text, because the $n^{\rm th}$ term is not in the form $A_n (x-c)^n$ required by 
(\eref{CLP101}{eq:SRradConv}). But we may still apply the ratio test with $a_n= (-1)^n n^2(x-a)^{2n}$.
Since
\begin{align*}
\lim_{n\to\infty} \bigg| \frac{a_{n+1}}{a_n} \bigg|
...
\begin{solution} (a)
We could determine the radius of convergence by applying (\eref{CLP101}{eq:SRradConv}), 
...
which is an alternating series when $x$ is nonzero. If we use its partial sum $S_N$ to approximate $\arctan x$, the absolute error involved is no more than
 \[\frac{|x|^{2(N+1)+1}}{2(N+1)+1} =\frac{|x|^{2N+3}}{2N+3}\]
 We want this error to be at most $10^{-5}$ whenever $-\frac14< x <\frac14$. For this range of $x$ values,
